...
//ADDING VARIABLEs FROM LAB 3
extern uint32_t volatile msTicks;
extern user_t *logged_in_user;
extern uint64_t sensor_states;
uint32_t counter = 0;
...
void system_update_state(alarm_system_t *system){
  switch (system->state){
    case UNARMED: 
      ...
      break;
    case WAITING_TO_ARM: 
			if(msTicks - counter < 10000 &&  logged_in_user == NULL){
				system->state = WAITING_TO_ARM;
			}
			else if(msTicks - counter >= 10000){
				system->state = ARMED;
			}
			else if(logged_in_user != NULL){
				system->state = UNARMED;
				logged_in_user = NULL;
			}
      break;
    case ARMED: 
			...
			else if(sensor_states != 0){
				system->state = ALARMED;
				__NVIC_SetPendingIRQ(EXTI2_IRQn);
			}
      break;
    case ALARMED: 
			if(logged_in_user != NULL && logged_in_user->is_super_user == 1){
				system->state = UNARMED;
				logged_in_user = NULL;
			}
      break;
